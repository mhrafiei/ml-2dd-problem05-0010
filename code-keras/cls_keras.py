...
class ClsFuns():
    ...
    def fun_run(self,datain,dataou,ind_train,ind_test):
        ...
        in_val = numpy.empty((datain.shape[0],datain.shape[1]*datain.shape[2]*datain.shape[3]))


        for q0 in range(datain.shape[0]):
            in_val[q0,:] = datain[q0,:,:,:].reshape((1,datain.shape[1]*datain.shape[2]*datain.shape[3]))

        print(in_val.shape)

        ind_val = numpy.sum(in_val, axis=0)
        ind_val = numpy.where(ind_val == 0)
        ind_val = ind_val[0]
        print(ind_val)
        print(type(ind_val))
        in_val = numpy.delete(in_val,ind_val,1)
